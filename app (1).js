...
// ─── LOAD ALL GUTENBERG BOOKS (fully paginated, no hard cap) ────────────────
// Gutendex returns 32 books per page. We follow every `next` URL until
// there are no more pages, across all romance-related topics.
// Small 200 ms pauses keep the browser UI responsive throughout.
async function load800GutenbergBooks() {
    const TOPICS = ['romance', 'love', 'courtship', 'marriage', 'woman'];
    let totalAdded = 0;

    scraperLogText.textContent = `[${new Date().toLocaleTimeString()}] 📡 Starting full Gutenberg sync — loading ALL available romance pages…`;

    for (const topic of TOPICS) {
        // Gutendex paginates via a `next` URL field; we follow it until null.
        ...
        while (nextUrl) {
            try {
                ...
                page++;
                // Gutendex supplies the full next-page URL directly
                nextUrl = data.next || null;

                // Update UI after every page batch
                syncedCount.textContent = appState.catalog.length;
                scraperLogText.textContent =
                    `[${new Date().toLocaleTimeString()}] 📥 Topic "${topic}" — page ${page}: +${batchAdded} books (library total: ${appState.catalog.length})`;
                renderGenreFilters();
                filterAndSortBooks();

                // 200 ms breathing room so the browser doesn't freeze
                await new Promise(r => setTimeout(r, 200));

            } catch (err) {
                console.warn(`Gutenberg fetch failed (topic=${topic}, page=${page}):`, err);
                break; // move to next topic on network error
            }
        }

        scraperLogText.textContent =
            `[${new Date().toLocaleTimeString()}] ✅ Topic "${topic}" fully synced — ${page} pages loaded.`;
    }

    // Persist the full catalog so the next visit loads instantly from cache
    try {
        const cachePayload = JSON.stringify(
            appState.catalog.map(b => ({ ...b, isFullyLoaded: false }))
        );
    ...
    scraperLogText.textContent =
        `[${new Date().toLocaleTimeString()}] 🎉 Full Gutenberg sync complete! ${totalAdded} new novels added — total library: ${appState.catalog.length} books.`;
    renderGenreFilters();
    filterAndSortBooks();
}
...
// 16. FULL BOOK PDF GENERATOR — ALL PAGES (CORS BYPASS + CHUNKED RENDERER)
// ─────────────────────────────────────────────────────────────────────────
// Strategy:
//   1. Strip Gutenberg header/footer from the raw text.
//   2. Wrap every text line to the PDF content width.
//   3. Render in async chunks (setTimeout) so the browser UI stays alive.
//   4. Every chunk writes one PDF page with a running header/footer.
//   5. On completion jsPDF saves the file to the user's Downloads folder.
//   6. If fetching fails for any reason, fall back to the preview PDF and
//      also open the Gutenberg ebook page so the user can grab it directly.
function downloadFullGutenbergPDF(book) {
    ...
    // ── Step 1: Fetch the full plain-text ──
    downloadStatusText.textContent  = 'Downloading full book text (all pages)…';
    downloadProgressPercentage.textContent = '10%';
    ...
    fetchFullTextWithFallback(gutenbergId)
        .then(rawText => {
            ...
            downloadStatusText.textContent = 'Stripping metadata headers & footers…';
            ...
            for (const m of startMarkers) {
                ...
                if (idx !== -1) { startIdx = rawText.indexOf('\n', idx) + 1; break; }
            }
            ...
            const bookText = rawText.substring(startIdx, endIdx);
            const rawLines = bookText.split(/\r?\n/);

            // ── Step 3: Set up jsPDF ──
            downloadStatusText.textContent = 'Setting up PDF document…';
            downloadProgressPercentage.textContent = '30%';
            downloadProgressFill.style.width = '30%';
            ...
            const LINE_H      = 6;       // mm between lines
            const BODY_TOP    = 22;      // mm — where body text starts (below header)
            const BODY_BOTTOM = pageH - 14; // mm — where body text ends (above footer)
            const LINES_PER_PAGE = Math.floor((BODY_BOTTOM - BODY_TOP) / LINE_H);

            const BURGUNDY = [30, 10, 18];
            ...
            doc.text('Lovestruck Hub', pageW / 2, pageH * 0.28, { align: 'center' });

            doc.setFont('times', 'bold');
            doc.setFontSize(24);
            doc.setTextColor(255, 255, 255);
            doc.text(book.title, pageW / 2, pageH * 0.45, { align: 'center', maxWidth: contentW });

            doc.setFont('times', 'italic');
            doc.setFontSize(16);
            doc.setTextColor(...GOLD);
            doc.text(`by ${book.author}`, pageW / 2, pageH * 0.54, { align: 'center' });

            doc.setFont('helvetica', 'normal');
            doc.setFontSize(9);
            doc.setTextColor(170, 170, 170);
            doc.text('COMPLETE PUBLIC DOMAIN EDITION — ALL PAGES', pageW / 2, pageH - 18, { align: 'center' });
            doc.text('via Project Gutenberg  ·  Lovestruck Hub', pageW / 2, pageH - 12, { align: 'center' });

            // ── Step 4: Wrap ALL lines to PDF width ──
            downloadStatusText.textContent = 'Wrapping all text lines to page width…';
            downloadProgressPercentage.textContent = '40%';
            downloadProgressFill.style.width = '40%';

            doc.setFont('times', 'normal');
            doc.setFontSize(11);

            const wrappedLines = [];
            // Process every single line — no artificial slice cap
            for (let i = 0; i < rawLines.length; i++) {
                const line = rawLines[i].trim();
                if (line === '') {
                    wrappedLines.push('');   // preserve paragraph breaks
                } else {
                    doc.splitTextToSize(line, contentW).forEach(s => wrappedLines.push(s));
                }
            }

            // ── Step 5: Chunked async page renderer ──
            let lineIdx = 0;
            let pageNum = 1;
            ...
            function renderNextPage() {
                if (lineIdx >= wrappedLines.length) {
                    ...
                    downloadStatusText.textContent = 'Saving PDF to Downloads folder…';

                    setTimeout(() => {
                        downloadProgressContainer.style.display = 'none';
                        const slug = book.title.toLowerCase().replace(/[^a-z0-9]+/g, '_');
                        doc.save(`lovestruck_${slug}_complete_all_pages.pdf`);
                        showToast(`✅ Full book saved: "${book.title}" (${pageNum - 1} pages)`, 'success');
                    }, 400);
                    return;
                }
                ...
                doc.setFontSize(7.5);
                doc.setTextColor(255, 255, 255);
                // Truncate title if very long so it fits the header
                const shortTitle = book.title.length > 55
                    ? book.title.substring(0, 52) + '…'
                    : book.title;
                doc.text(`${shortTitle}  ·  ${book.author}`, margin, 7);
                ...
                let y = BODY_TOP;
                const pageEnd = Math.min(lineIdx + LINES_PER_PAGE, wrappedLines.length);

                for (let i = lineIdx; i < pageEnd; i++) {
                ...
                doc.text('Lovestruck Hub  ·  lovestruck-hub.com', pageW / 2, pageH - 6, { align: 'center' });

                lineIdx = pageEnd;
                pageNum++;

                // Progress bar update (40 → 95 %)
                const pct = 40 + Math.floor((lineIdx / wrappedLines.length) * 55);
                downloadProgressPercentage.textContent = `${pct}%`;
                downloadProgressFill.style.width = `${pct}%`;
                downloadStatusText.textContent = `Rendering page ${pageNum} of ~${Math.ceil(wrappedLines.length / LINES_PER_PAGE) + 1}…`;

                // Yield to the browser every page so the UI stays responsive
                setTimeout(renderNextPage, 4);
            }

            // Kick off page rendering
            setTimeout(renderNextPage, 80);
        })
        .catch(err => {
            console.error('Full-text PDF download failed:', err);
            downloadStatusText.textContent = 'CORS fetch blocked — falling back to preview…';

            setTimeout(() => {
                downloadProgressContainer.style.display = 'none';
                showToast('Direct fetch blocked. Opening Gutenberg page + downloading preview PDF.', 'warning');
                generateRealPDF(book);
