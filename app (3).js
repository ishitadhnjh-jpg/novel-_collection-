...
// For every book: attempts to fetch the complete plain-text from Gutenberg
// via a CORS proxy and compile it into a fully paginated PDF (all pages).
// Falls back to a rich preview PDF if the proxy is blocked.
function startSmartDownload(book, format) {
    // For EPUB: always open the real Gutenberg EPUB link if available, else mock file
    if (format === 'EPUB') {
...
function downloadFullGutenbergPDF(book) {
    ...
    async function tryProxies(index) {
        if (index >= proxies.length) {
            // All proxies failed — open Gutenberg page directly for manual download
            downloadProgressContainer.style.display = 'none';
            showToast(`Network blocked. Opening Gutenberg page for direct download.`, 'warning');
            window.open(`https://www.gutenberg.org/ebooks/${gutenbergId}`, '_blank');
            generateRealPDF(book); // Still give them a preview PDF
            return;
        }
