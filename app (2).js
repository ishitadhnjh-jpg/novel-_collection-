...
// CORS proxies: routes requests through a middleman so the browser doesn't block them
const CORS_PROXIES = [
    ...
    for (const proxyFn of CORS_PROXIES) {
        ...
        for (let i = 0; i < attempts; i++) {
            try {
                ...
                if (i === attempts - 1) break; // try next proxy
...
async function load800GutenbergBooks() {
    ...
    for (let p = 1; p <= MAX_PAGES; p++) {
        for (const topic of TOPICS) {
            tasks.push(async () => {
                if (appState.catalog.length >= 950) return; // Stop loading if target reached
    ...
    async function runConcurrent(taskFns, limit) {
        ...
        for (const task of taskFns) {
            if (appState.catalog.length >= 950) break; // Early stop
            const p = task().then(r => {
